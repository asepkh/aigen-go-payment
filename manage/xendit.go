...
func (m *Manager) ProcessXenditInvoicesCallback(ctx context.Context, ips *xendit.InvoicePaymentStatus) error {
	...
	if ips.RecurringPaymentID != "" {
		err = m.processXenditRecurringTransactionCallback(ctx, ips)
	} else {
		err = m.processXenditNonRecurringTransactionCallback(ctx, ips)
	}
	...
	// Get the invoice to pass to the callback
	inv, err := m.GetInvoice(ctx, ips.ExternalID)
	if err == nil && m.paymentCallbackProcessedCallback != nil {
		go func() {
			err := m.paymentCallbackProcessedCallback(context.Background(), inv)
			if err != nil {
				l.Warn().
					Err(err).
					Msg("failed sending payment callback processed callback")
			}
		}()
	}
